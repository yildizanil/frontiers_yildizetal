...
c_topo = ax1.contourf(dem, extent=[0, 5000, 0, 4000],
                      cmap='Greys', zorder=0,
                      vmin=0, vmax=1400)
ax1.vlines(x=3000, ymin=0, ymax=4000,
           zorder=0, color='k',
           linestyles='dotted')
c1 = ax1.imshow(hmax_ma, cmap='viridis',
                extent=[0, 5000, 0, 4000],
                zorder=1, vmin=0.1, vmax=20)
fig.colorbar(c1,ax=ax1,location='top',
             orientation='horizontal',
             label='Maximum flow height [m]')

fig.subplots_adjust(bottom=0.5)
cbar_ax = fig.add_axes([0.05, 0, 0.90, 0.05])
fig.colorbar(c_topo, cax=cbar_ax, cmap=cmap, norm=norm,
             orientation='horizontal',
             label='Elevation [m]', shrink=0.36)
...
ax2.contourf(dem, extent=[0, 5000, 0, 4000],
             cmap='Greys', zorder=0,
             vmin=0, vmax=1400)
ax2.vlines(x=3000, ymin=0, ymax=4000,
           zorder=0, color='k',
           linestyle='dotted')
c2 = ax2.imshow(hfin_ma, cmap='viridis',
                extent=[0, 5000, 0, 4000],
                zorder=1, vmin=0.1, vmax=4)
fig.colorbar(c2, ax=ax2, location='top',
             orientation='horizontal',
